...
- 価格表示：
  - PRICE_PRODUCT_MAPから product_code が特定でき、PRICE_SPECで金額が確定し、monitor_typeが確定した場合のみ表示（SHOW）
  - 不明ならHIDEにし、料金ページ誘導のみ（間違うより出さない）
  - 価格免責：「＊本ページ記載のモニター制度・料金は投稿時点の内容です。今後、内容が変更・終了となる場合があります。」を必ず入れる
